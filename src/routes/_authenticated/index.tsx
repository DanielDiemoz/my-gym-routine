...
import { Flame, TrendingUp, TrendingDown, Dumbbell, ChevronRight, LogOut } from "lucide-react";

export const Route = createFileRoute("/_authenticated/")({
  component: Dashboard,
});

type SessionRow = { id: string; started_at: string; completed_at: string | null; total_volume: number; plan_name: string | null };
type LogRow = { muscle_group: string | null; reps: number; weight: number; session_id: string; created_at: string };
  ...
  return (
    <div className="container-app pt-10">
      <header className="mb-8 flex items-start justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">{format(new Date(), "EEEE d MMM", { locale: it })}</p>
          <h1 className="mt-1 text-3xl font-black tracking-tight">Ciao, {name.split(" ")[0]}</h1>
        </div>
        <button
          onClick={async () => { await supabase.auth.signOut(); window.location.replace("/auth"); }}
      ...
      <section className="rounded-3xl bg-primary p-6 text-primary-foreground">
        <p className="text-xs font-semibold uppercase tracking-widest opacity-70">Questa settimana</p>
        <div className="mt-2 flex items-end justify-between">
          <div className="text-6xl font-black tracking-tighter">{stats?.workouts ?? 0}</div>
          <div className="pb-2 text-sm opacity-70">{stats?.workouts === 1 ? "allenamento" : "allenamenti"}</div>
        </div>

        <div className="mt-6 grid grid-cols-7 gap-1.5">
          {stats?.days.map((d, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <div className={`h-9 w-full rounded-lg ${d.trained ? "bg-primary-foreground" : "bg-primary-foreground/15"}`} />
              <span className="text-[10px] font-semibold uppercase opacity-60">{format(d.date, "EEEEE", { locale: it })}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-4 grid grid-cols-2 gap-3">
        <StatCard
          label="Volume tot."
          value={`${Math.round(stats?.volume ?? 0).toLocaleString("it-IT")} kg`}
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-baseline justify-between">
          ...
          <Link to="/schede" className="text-xs font-semibold text-muted-foreground">Tutte →</Link>
        </div>
        {plansQ.data && plansQ.data.length === 0 ? (
          <Link to="/schede" className="block rounded-2xl border-2 border-dashed border-border p-6 text-center">
          ...
          <div className="space-y-2">
            {plansQ.data?.map((p) => (
              <Link
                ...
                to="/allena/$planId"
                params={{ planId: p.id }}
                className="no-tap-highlight flex items-center justify-between rounded-2xl bg-card border border-border px-5 py-4 transition active:scale-[0.99]"
              >
      ...
      {stats && stats.recent.length > 0 && (
        <section className="mt-8">
          ...
          <div className="space-y-2">
            {stats.recent.map((s) => (
              <div key={s.id} className="flex items-center justify-between rounded-2xl bg-card border border-border px-5 py-3.5">
                <div>
                  <div className="text-sm font-semibold">{s.plan_name ?? "Allenamento"}</div>
                  <div className="text-xs text-muted-foreground">{format(new Date(s.started_at), "EEE d MMM", { locale: it })}</div>
                </div>
                <div className="text-sm font-bold">{Math.round(Number(s.total_volume)).toLocaleString("it-IT")} kg</div>
              </div>
            ))}
          </div>
        </section>
...
function StatCard({ label, value, trend, icon }: { label: string; value: string; trend?: number; icon?: React.ReactNode }) {
  return (
    <div className="rounded-2xl bg-card border border-border p-4">
      ...
      {trend !== undefined && (
        <div className={`mt-1 flex items-center gap-1 text-xs font-semibold ${trend >= 0 ? "text-[oklch(0.55_0.16_145)]" : "text-destructive"}`}>
          {trend >= 0 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
          {trend >= 0 ? "+" : ""}{trend.toFixed(0)}% vs settimana scorsa
        </div>
      )}
    </div>
  );
